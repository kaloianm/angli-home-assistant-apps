...
class TestExtractorFanPairLogic(unittest.TestCase):
    ...
    def test_activated_but_short_visit_turns_off_immediately(self):
        ...
        actions_activation = self.logic.on_time_tick(self.t0 +
    ...
    def test_long_visit_keeps_fan_for_same_duration_after_light_off(self):
        ...
        actions_before_deadline = self.logic.on_time_tick(self.t0 + timedelta(
            seconds=239))
        self.assertNotIn(ACTION_FAN_OFF, _kinds(actions_before_deadline))

        actions_at_deadline = self.logic.on_time_tick(self.t0 +
    ...
    def test_schedule_and_occupancy_overlap_uses_latest_end(self):
        schedule_actions = self.logic.on_schedule_started(self.t0,
        ...
        actions_before_schedule_end = self.logic.on_time_tick(self.t0 +
                                                              timedelta(
                                                                  seconds=299))
        self.assertNotIn(ACTION_FAN_OFF, _kinds(actions_before_schedule_end))

        actions_schedule_end = self.logic.on_time_tick(self.t0 +
    ...
    def test_manual_override_is_authoritative_until_full_cycle_reset(self):
        ...
        manual_off = self.logic.on_manual_fan_toggle(self.t0 +
                                                     timedelta(seconds=1),
                                                     fan_on=False)
        ...
        still_blocked = self.logic.on_time_tick(self.t0 +
    ...
    def test_deadline_timer_tracks_nearest_expiration(self):
        ...
        timer_set_actions = self.logic.on_time_tick(self.t0 +
        ...
        self.assertTrue(
            any(a.at == self.t0 + timedelta(seconds=190)
                for a in deadline_sets) or timer_set_actions == [])
