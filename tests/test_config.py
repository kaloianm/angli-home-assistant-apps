...
class TestConfigParsing(unittest.TestCase):
    ...
    def test_missing_required_pair_field_is_rejected(self):
        with self.assertRaisesRegex(
            ValueError, "pairs\\[0\\]\\.daily_run_duration_seconds is required"
        ):
            parse_app_config(
                {
                    "pairs": [
    ...
    def test_pulse_guard_must_be_smaller_than_staircase_interval(self):
        with self.assertRaisesRegex(
            ValueError, "pulse_guard_seconds must be smaller"
        ):
            parse_app_config(
                {
